...
# Get the stock quote
df = pdr.get_data_yahoo(ticker, start = start_date, end=datetime.now())
st.write(df.tail()) 
...
def LSTM_model():
    fig = go.Figure()
    fig.add_trace(go.Scatter(x = train.index,y = train['Close'],name = f'{ticker} 10 day Moving Average'))
    fig.add_trace(go.Scatter(x = validate.index,y = validate[['Close','Predictions']], name = f'{ticker} 20 day Moving Average'))
...
st.write(Next_30days_forecast.head(10)) 
